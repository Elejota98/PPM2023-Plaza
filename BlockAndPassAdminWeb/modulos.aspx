<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="modulos.aspx.cs" Inherits="BlockAndPass.AdminWeb.modulos" %>
    ...
    <head>
	    ...
	    <script type="text/javascript">
	        ...
	        function CargarModulos() {
	            $.ajax({
	                type: "GET",
	                url: "DataService.asmx/GetItemsModulos",
	                data: "",
	                contentType: "application/json",
	                dataType: "json",
	                success: function (msg) {
	                ...
	                error: function () {
	                    alert("Failed to load detalle modulos");
	                }
	            });
	        }
	        function fillmodulos(msg) {
	        ...
	        function btnEditar_click() {
	            document.getElementById('idModuloModal').disabled = true;
	            var table = $('#sampleTable').DataTable();
	            if (table.rows({ selected: true }).count() > 0) {
	                var count = table.rows({ selected: true }).data();
	                ////alert(count[0][1]);
	                $('#idModuloModal').val(count[0][1]);
	                $('#idEstacionamientoModal').val(count[0][2]);
	            
	                $('#ipModal').val(count[0][3]);
	                $('#carrilModal').val(count[0][4]);
	                $('#idTipoModuloModal').val(count[0][5]);

	                $('#extencionModal').val(count[0][6]);
	                $("#macEstacionamientoModal").val(count[0][10]);
	        ...
	        function ConsultaTipoModulo() {
	            $.ajax({
	                type: "GET",
	                url: "DataService.asmx/ObtenerListaTipoModulo",
	                data: "{}",
	                contentType: "application/json",
	                dataType: "json",
	                success: function (msg) {
	                    $("#idTipoModuloModal").get(0).options.length = 0;

	                    //$("#cbEstacionamiento").get(0).options[$("#cbEstacionamiento").get(0).options.length] = new Option('Todos', 0);
	                    $.each(msg.d, function (index, item) {
	                        $("#idTipoModuloModal").get(0).options[$("#idTipoModuloModal").get(0).options.length] = new Option(item.Display, item.Value);
	                    });
	                },
	                error: function () {
	                    $("#idTipoModuloModal").get(0).options.length = 0;
	                    alert("Failed to load tipomodulo");
	                }
	        ...
	        function btnModalGuardar_click() {
	            if ($('.modal-title').html() == 'Crear') {
	                ...
	                $.ajax({
	                    type: "GET",
	                    url: "DataService.asmx/CrearModulo?idEstacionamiento=" + $("#idEstacionamientoModal").val() + "&ip=" + $("#ipModal").val() + "&carril=" + $("#carrilModal").val() + "&idTipoModulo=" + $("#idTipoModuloModal").val() + "&extension=" + $("#extencionModal").val() + "&estado=" + activo + "&idModulo=" + $("#idModuloModal").val() + "&mac=" + $("#macEstacionamientoModal").val(),
	                    data: "",
	                    contentType: "application/json",
	                    dataType: "json",
	                    success: function (msg) {
	                ...
	                $.ajax({
	                    type: "GET",
	                    url: "DataService.asmx/ActualizarModulo?idEstacionamiento=" + $("#idEstacionamientoModal").val() + "&ip=" + $("#ipModal").val() + "&carril=" + $("#carrilModal").val() + "&idTipoModulo=" + $("#idTipoModuloModal").val() + "&extension=" + $("#extencionModal").val() + "&estado=" + activo + "&idModulo=" + $("#idModuloModal").val() + "&mac=" + $("#macEstacionamientoModal").val(),
	                    data: "",
	                    contentType: "application/json",
	                    dataType: "json",
	                    success: function (msg) {
	        ...
	        function btnCrear_click() {
	            document.getElementById('idModuloModal').disabled = false;
	            $('#idModuloModal').val('');
                        ...
                        <nav class="navbar navbar-default" id="myNav">
                            <div class="collapse navbar-collapse">
                                <ul class="nav navbar-nav nav-tabs-justified">
                                    ...
                                    <li><a href="estacionamientos.aspx">Estacionamientos</a></li>
                                    <li class="active"><a href="#">Modulos</a></li>
                                            ...
                                            <textarea id="Template-Items2" rows="0" cols="0"><!--
                                                 <div class="content table-responsive table-full-width">  
                                                    <table id="sampleTable" class="widthFull fontsize10 displayNone">
                                                        ...
                                                        <tr>
                                                            <td></td>
                                                            <td>{$T.post.IdModulo}</td>
                                                            <td>{$T.post.IdEstacionamiento}</td>
                                                            <td>{$T.post.IP}</td>
                                                            <td>{$T.post.Carril}</td>
                                                            <td>{$T.post.IdTipoModulo}</td>
                                                            <td>{$T.post.Extension}</td>
                                                            <td>{$T.post.TipoModulo}</td>
                                                            <td>{$T.post.Nombre}</td>
                                                            <td>{$T.post.Estado}</td>
                                                            <td>{$T.post.Mac}</td>
                                                        </tr>
                        ...
                        <div class="content">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>IP</label>
                                        <input type="text" class="form-control border-input" id="ipModal">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>Carril</label>
                                        <input type="text" class="form-control border-input" id="carrilModal">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>IdTipoModulo</label>
                                        <select class="form-control border-input" id="idTipoModuloModal">
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control border-input" id="extencionModal">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>MAC</label>
                                        <input type="text" class="form-control border-input" id="macEstacionamientoModal">
                                    </div>
                                </div>
                            </div>    
                        </div>
